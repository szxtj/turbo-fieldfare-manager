...
- **最大上下文长度**：全面适配官方 v0.9.0，完整支持 4K、8K、16K、32K（推荐）、64K、96K、128K、192K 及 262K（256K 模型物理极限）；
- **显存准入与强制覆盖**：支持一键开启“允许超出物理显存预算（`TURBO_FIELDFARE_ALLOW_UNBACKED_CONTEXT=1`）”，防止 8GB/16GB 设备因上下文过大被服务端拒绝启动；
- **官方 v0.9.0 算子加速**：自动适配编译最新的 Grouped Full Attention（KV-15 算子），超长文本解码提速 2.21 倍（达 18 tok/s）；
